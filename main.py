...
ennemi_change_base = [3, 100,3,50,3, 100,3,100,3, 100,3,100,3, 100,340]  # Déplacement x et y
# Le sens de demarage est décidé au hasard
if randint(1, 2) == 1:
    ennemi_change = ennemi_change_base.copy()
else:
    ennemi_change = [-3, 50, -3, 100, -3, 50, -3, 100, -3, 50, -3, 100, -3, 100, 340]
...
# Divers variables/images utiles au jeux.
score_temp = 0  # Le score du joueur
vie = 3         # Nombres de vie de base du joueur
vie_image = charger.load("assets/vies.png")    # Chargement de l'image des vies

# Image coordonnees et etat de la recompense
recompense_image = charger.load("assets/recompense.png")
recompense_coordonees = [0,0]
...
# Dessin des vies
def affichage_vies(x,y):
    screen.blit(vie_image,(x,y))

# Dessin de la recompense
def recompense(co):
    screen.blit(recompense_image,(co[0],co[1]))
# Horloge permettant de définir la vitesse de rafraichissement de la page
clock = pygame.time.Clock()
# Etat du jeu (game over ou pas)
game_running = True
    ...
    if game_running == True :
        ...
        if ennemi[1] == 1000 and ennemi[3]== 1000 and ennemi[5]== 1000 and ennemi[7]== 1000:
            ...
            if randint(1,2) == 1:
                ...
                ennemi_change = [-3, 100,-3,50,-3, 100,-3,100,-3, 50,-3,100,-3, 100,340]
        ...
        if bombe_active == False:
            ...
            # Dessin et mouvement de la bombe
            ...
            if spaceship[0]<=bombe_coordonnées[0]<= spaceship[0] + 64 and spaceship[1] < bombe_coordonnées[1] < spaceship[1] + 64:
                bombe_active = False
                vie -= 1                # Quand la bombe touche le joueur il perds une vie
                hit = pygame.mixer.Sound("assets/hit.wav")
                hit.play()
            elif bombe_coordonnées[1] > 600:
                bombe_active = False
        # Creation de la recompense 1 fois sur 2000 execution en moyenne
        if recompense_active == False:
            if randint(1, 2000) == 12:
                recompense_active = True
                recompense_coordonees[0] = randint(0,739)
                recompense_coordonees[1] = randint(0,100)
        # Mouvmeent et collision de l'etoile
        else:
            # Mouvement
            recompense_coordonees[1] += recompense_change
            recompense(recompense_coordonees)
            # Sortie de l'ecran
            if recompense_coordonees[1] > 600:
                recompense_active = False
            # COllision avec le joueur
            if spaceship[0]-59<=recompense_coordonees[0]<= spaceship[0] + 64 and spaceship[1]-50 < recompense_coordonees[1] < spaceship[1] + 64:
                nb_projectiles = 100
                recompense_active = False
                pickup = pygame.mixer.Sound("assets/pickup.wav")
                pickup.play()

        dessin_du_score(10, 15)
        nombre_restant_projectiles(10, 45)
        # Dessin de la vie restante par rapport au nombre de vies qu'on as.
        for u in range(vie):
            affichage_vies(10+30*u,75)
        if vie == 0:
            game_running = False
        for h in range(1,8,2):
            print(ennemi[h])
            if ennemi[h] > 550 and ennemi[h] < 800:
                vie = 0
    # Game Over
    else:
        ...
        if repetition_son_gameover == 0:
            lose.play()
            repetition_son_gameover += 1
        # Ecriture du Game Over
        screen.fill((0,0,0))
        font = pygame.font.Font('assets/Minecraft.ttf', 100)
        game_over = font.render("Game Over", True, (255, 255, 255))
        screen.blit(game_over,(120,235))
